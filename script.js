document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize Event Listeners for menu etc. inside initializeApp
  // initializeApp will be called after login/auth check
});
...
function initializeApp() {
  // BARU: Daftarkan Plugin Zoom (Modifikasi A)
  Chart.register(ChartZoom);

  // BARU: Inisialisasi Modal (Modifikasi A & Poin C)
  ...
  // BARIS BARU DITAMBAHKAN: Inisialisasi Info Modal
  if (document.getElementById('infoModal')) { 
      infoModalInstance = new bootstrap.Modal(document.getElementById('infoModal')); 
  }
  // (Listener 'hidden.bs.modal' untuk zoomModal sekarang ada di handleFullscreenClick)
  ...
  document.getElementById('menu-export-excel')?.addEventListener('click', handleExportExcel); // BARU: Poin D
  ...
  // Listener Tombol Menu Utama
  document.getElementById('menu-analisis')?.addEventListener('click', () => showSection('analisis-section'));

  // Listener Tombol Kembali (Gabungkan dengan yang sudah ada)
  document.querySelectorAll('[id^="back-to-dashboard"], #back-analisis-to-dashboard').forEach(button => {
    button?.addEventListener('click', () => showSection('dashboard-section'));
  });
  
  // Listener Tombol Kembali di Halaman Detail Analisis
  ...
  // Listener Tombol Sub-Menu Analisis
  ...
  // ===== TAMBAHKAN BLOK INI =====
  // Pasang listener untuk daftar pengeluaran (ul#expenses-ul) SEKALI saja
  const expensesUl = document.getElementById('expenses-ul');
  if (expensesUl) {
      // Hapus listener lama jika ada (mencegah duplikasi jika initializeApp terpanggil lagi secara tidak sengaja)
      expensesUl.removeEventListener('click', handleExpenseListClick);
      // Tambahkan listener baru yang memanggil fungsi handler
      expensesUl.addEventListener('click', handleExpenseListClick);
      console.log("Event listener untuk daftar pengeluaran (expenses-ul) berhasil dipasang."); // Pesan konfirmasi (opsional)
  } else {
      // Ini seharusnya tidak terjadi jika HTML benar, tapi bagus untuk pengecekan
      console.error("Elemen 'expenses-ul' tidak ditemukan saat inisialisasi!");
  }
  ...
  // BARU: Set default Tanggal & Jam di Form Tambah (Poin D)
  setDefaultDateTime();
  ...
  // BARU: Muat Sumber Dana yang tersimpan terlebih dahulu (Poin D)
  console.log("Loading saved categories on init..."); // Log
  loadSavedCategories();
  // Handle Kategori Dropdown
  ...
  // Initialize Tips Modal (pastikan instance dibuat)
  ...
  // BARU: Event listener untuk toggle garis rata-rata (Poin C)
      ...
      toggleAvgButton.addEventListener('click', (e) => {
          if (dailyExpenseChart) {
              ...
              button.innerHTML = newState 
                ? '<i class="fas fa-check-circle"></i> Garis Rata-rata' 
                : '<i class="fas fa-times-circle"></i> Garis Rata-rata';
              
              // Update chart
              dailyExpenseChart.setDatasetVisibility(1, newState); // Dataset 1 adalah rata-rata
              dailyExpenseChart.update();
          }
      });
  }
  
  // BARU: Event listener untuk tombol animasikan (Poin C)
  const animateButton = document.getElementById('animate-daily-chart');
  if (animateButton) {
      animateButton.addEventListener('click', animateDailyChart);
  }
  
  // BARU: Event listener untuk tombol fullscreen (delegated) (Modifikasi A)
  document.addEventListener('click', (e) => {
  ...
  // ===============================================
  // Terapkan perbaikan visual dinamis saat aplikasi dimulai
  applyDynamicStyles();
  // ===============================================
}
...
function showSection(sectionId) {
  console.log(`Navigating to section: ${sectionId}`); // Log
  ...
  if (sectionId === 'dashboard-section' || sectionId === 'analisis-section') {
      ...
      resetAnalysisViews(); // Panggil fungsi reset
  }

  // Render/Update konten sesuai Section (Gunakan switch-case)
  switch (sectionId) {
    ...
    case 'history-expense-section':
      initializeHistoryYearOptions();
      if(document.getElementById('history-content')) document.getElementById('history-content').innerHTML = '<p class="text-muted">Pilih tahun dan bulan.</p>';
      break;
    ...
    // ===== KASUS BARU UNTUK ANALISIS =====
    case 'analisis-section':
      // Tidak perlu load data di sub-menu, biarkan kosong
      break;
    case 'anomali-detail-section':
      loadAndRunAnomalyDetection(); // Panggil fungsi baru
      break;
    case 'prediksi-detail-section':
      loadAndSetupPrediction(); // Panggil fungsi baru
      break;
    // ===================================
  }
...
function handleAddExpense(e) {
  ...
  const kategoriInput = document.getElementById('kategori-selected'); // Hanya kategori
  const currentUser = localStorage.getItem('currentUser');

  // Hapus pengecekan sumberDanaInput
  if (!dateInput || !timeInput || !barangInput || !amountInput || !kategoriInput || !currentUser) {
      console.error("Missing required expense form elements."); // Log error jika elemen hilang
      showNotification('Terjadi kesalahan pada form.', 'danger');
      return;
  }
  ...
  let amount = parseFloat(amountInput.value.trim()); // Ambil nilai amount
  const kategori = kategoriInput.value.trim();
  // Hapus variabel sumberDana

  // Validasi input (Hapus 'sumberDana') - Tambahkan pengecekan amount > 0
  if (barang && !isNaN(amount) && amount > 0 && kategori && tanggal && jam) {
    ...
    // Buat objek expense (Hapus 'sumberDana')
    const expense = {
      id: Date.now(),
      barang,
      kategori,
      amount: amount, // Pastikan amount adalah angka
    ...
    let expenses = []; // Deklarasikan di luar try-catch
    try {
        expenses = JSON.parse(localStorage.getItem(expensesKey)) || [];
        if (!Array.isArray(expenses)) expenses = []; // Pastikan array
    } catch (error) {
        console.error("Error parsing expenses from localStorage:", error);
        expenses = []; // Reset jika ada error parsing
    }
    ...
    if (form) form.reset(); // Reset form jika elemen ditemukan
    // Reset tombol dropdown kategori
    const katButton = document.getElementById('kategori-button');
    if (katButton) katButton.innerHTML = `Pilih Kategori <i class="fas fa-chevron-down float-end"></i>`;
    // Hapus reset tombol sumber dana
    setDefaultDateTime(); // Setel ulang tanggal/jam

    // Render ulang list jika perlu
    if (expenseMonth === getCurrentMonth() && expenseYear === getCurrentYear()) {
        renderExpensesList();
    }

    // Update opsi di halaman lain
    ...
    if (isNaN(amount) || amount <= 0) {
        errorMessage = 'Jumlah pengeluaran harus berupa angka positif.';
    } else if (!kategori) {
        errorMessage = 'Kategori belum dipilih.';
    } else if (!barang) {
        errorMessage = 'Nama barang belum diisi.';
    } else if (!tanggal || !jam) {
        errorMessage = 'Tanggal atau jam belum diisi.';
    }
    showNotification(errorMessage, 'warning'); // Gunakan tipe 'warning'
  }
}
...
function renderExpensesList() {
  ...
  // Urutkan expense berdasarkan tanggal terbaru dulu
  ...
  for (const [dateKey, itemsOnDate] of Object.entries(groupedByDate)) {
      ...
      dateLi.style.backgroundColor = '#f0f0f0'; // Latar abu-abu untuk tanggal
      ...
      subUl.classList.add('ms-3', 'mt-2', 'mb-2', 'd-none'); // Mulai tertutup

      // Tampilkan item dalam tanggal itu
      // Tampilkan item dalam tanggal itu
      // Tampilkan item dalam tanggal itu
      itemsOnDate.forEach(exp => {
          const itemLi = document.createElement('li');
          itemLi.className = 'list-group-item d-flex justify-content-between align-items-center';
          // ===== PERBAIKAN: Hapus Kategori dari Tampilan =====
          itemLi.innerHTML = `
              <span>
      ...
      expensesUl.appendChild(subUl); // Tambahkan sub-list ke list utama
  } // Ini menutup loop 'for...of Object.entries(groupedByDate)' dari bagian sebelumnya

  // --- Pasang Event Listener ---

  // Pasang Event Listener untuk Toggle Bagian Tanggal (Tidak perlu diubah)
}
...
function handleDeleteExpenseById(e) {
  ...
  let deletedItemInfo = ""; // Untuk notifikasi
    ...
    if (key.startsWith(`expenses_${currentUser}_`)) {
      ...
      expensesInMonth = expensesInMonth.filter(exp => {
          if (exp.id === idToDelete) {
              deletedItemInfo = `"${capitalizeFirstLetter(exp.barang)}"`; // Simpan info barang
              return false; // Jangan masukkan item ini ke array baru
          }
          return true; // Masukkan item lain
      });

      // Jika ada item yang dihapus dari bulan ini
      if (expensesInMonth.length < initialLength) {
        if (expensesInMonth.length > 0) {
          ...
          localStorage.removeItem(key); // Hapus key jika bulan jadi kosong
        }
        expenseDeleted = true;
...
function renderDashboard() {
  ...
  initializeCategoryExpenseEnhanced(); // Panggil ini untuk inisialisasi dropdown kategori enhanced
}
...
function loadAndRunAnomalyDetection() {
    ...
    setTimeout(() => {
        ...
        if (expensesForCurrentMonth.length < 5) {
             ...
             if (noDataEl) {
                noDataEl.style.display = 'flex';
                // Gunakan teks dari HTML Anda (atau ganti jika perlu)
        ...
        // ========================================================
        // PERUBAHAN DI SINI: Teruskan 'categoryBounds'
        // ========================================================
...
function renderAnomalyChart(anomalies, currentMonthExpenses, categoryBounds) {
    ...
    console.log("Processing expenses for anomaly chart:", currentMonthExpenses.length);
    ...
    currentMonthExpenses.forEach(exp => {
        try {
            ...
            // --- PERUBAHAN: Dapatkan info batas untuk disimpan di data titik ---
            ...
            const dataPoint = {
                x: expenseDate.getTime(),
                y: expenseAmount,
                label: `${capitalizeFirstLetter(exp.barang || 'Unknown')} (${capitalizeFirstLetter(exp.kategori || 'Unknown')})`,
                // --- DATA BARU UNTUK FITUR KLIK ---
                category: capitalizeFirstLetter(exp.kategori || 'Unknown'),
                normalLower: bounds ? bounds.lower : 0,
                normalUpper: bounds ? bounds.upper : Infinity // Jika tidak ada batas, anggap tak terhingga
            };
            // --- AKHIR DATA BARU ---
    ...
    console.log("Chart data prepared - Normal:", normalData.length, "Anomaly:", anomalyData.length);

    if (normalData.length === 0 && anomalyData.length === 0) {
        // ... (kode error handling jika tidak ada data, biarkan sama) ...
        return;
    }
    ...
    try {
                ...
                datasets: [
                    {
                        label: 'Normal',
                        data: normalData,
                        // --- TAMPILAN ADVANCE (BIRU) ---
                        backgroundColor: 'rgba(15, 12, 227, 0.7)',
                        borderColor:  'rgba(15, 12, 227, 0.7)',
                        borderWidth: 1,
                        pointRadius: 5, 
                        pointHoverRadius: 7,
                        pointStyle: 'circle' // Gaya titik normal
                    },
                    {
                        label: 'Anomali',
                        data: anomalyData,
                        // --- TAMPILAN ADVANCE (MERAH) ---
                        backgroundColor: 'rgba(255, 0, 25, 0.7)',
                        borderColor: 'rgba(255, 0, 25, 0.7)',
                        borderWidth: 1,
                        pointRadius: 8, // Sedikit lebih besar
                        pointHoverRadius: 10,
                        pointStyle: 'triangle' // <-- Tampilan Bintang
                    }
                ]
            },
            options: {
                ...
                scales: {
                    x: {
                        ...
                        // --- TAMPILAN ADVANCE (Grid Halus) ---
                        grid: {
                            color: '#e9ecef',
                            drawBorder: false,
                            borderDash: [2, 3] // Garis putus-putus
                        }
                    },
                    y: {
                        beginAtZero: true,
                ...
                // ========================================================
                // PERUBAHAN: Menambahkan onClick dan onHover
                // ========================================================
                onClick: (evt, activeElements) => {
                    if (activeElements.length > 0) {
                        const dataIndex = activeElements[0].index;
                        ...
                        if (clickedPoint) {
                            ...
                            const title = `Batas Normal: ${category}`;
                            const message = `Rentang wajar untuk kategori ini adalah antara <br><strong>Rp${lower.toLocaleString('id-ID')}</strong> dan <strong>Rp${upper.toLocaleString('id-ID')}</strong>.`;
                ...
                onHover: (event, chartElement) => {
                    // Mengubah kursor menjadi 'pointer' saat di atas titik
                    const canvas = event.native.target;
                    canvas.style.cursor = chartElement[0] ? 'pointer' : 'default';
                }
                // ========================================================
        ...
        console.log("Anomaly chart created successfully");
    } catch (error) {
